              ...
              <div className="p-6 bg-white rounded-lg shadow-md border border-gray-200">
                  ...
                  <button
                    onClick={async (e) => {
                      ...
                      if (confirm(`Are you sure you want to delete "${activeDoc.docName}"?`)) {
                        try {
                          const res = await fetch(`http://localhost:5000/document_create/${activeDoc.id}`, {
                            method: "DELETE",
                          });
                          const data = await res.json();
                ...
                <div className="mt-4 border rounded p-2 bg-gray-50">
                  {/\.(jpg|jpeg|png|gif|webp)$/i.test(activeDoc.fileName) ? (
                    // Images
                    <img
                      src={`http://localhost:5000${activeDoc.filePath}`}
                    ...
                    <iframe
                      src={`https://docs.google.com/gview?url=http://localhost:5000${activeDoc.filePath}&embedded=true`}
                      className="w-full h-[500px] border rounded"
                    ></iframe>
                    ...
                    <div className="text-center p-4">
                      ...
                      <a
                        href={`http://localhost:5000${activeDoc.filePath}`}
                        download
